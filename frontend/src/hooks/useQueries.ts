...
export function useAdminRegister() {
  ...
  return useMutation<RegistrationStatus, Error, { roleText: string; name: string }>({
    mutationFn: async ({ roleText, name }) => {
      if (!actor) throw new Error('Actor not available');
      try {
        const result = await actor.adminRegister(roleText, name);
        return result;
      } catch (error) {
        throw new Error(normalizeBackendError(error));
      }
...
export function useDriverRegister() {
  ...
  return useMutation<RegistrationStatus, Error, { name: string; busNumber: string }>({
    mutationFn: async ({ name, busNumber }) => {
      if (!actor) throw new Error('Actor not available');
      try {
        // Backend signature: driverRegister(name: string, busNumber: string)
        const result = await actor.driverRegister(name, busNumber);
        return result;
      } catch (error) {
        throw new Error(normalizeBackendError(error));
      }
...
export function useTravellerRegister() {
  ...
  return useMutation<RegistrationStatus, Error, { name: string }>({
    mutationFn: async ({ name }) => {
      if (!actor) throw new Error('Actor not available');
      try {
        // Backend signature: travellerRegister(name: string)
        const result = await actor.travellerRegister(name);
        return result;
      } catch (error) {
        throw new Error(normalizeBackendError(error));
      }
